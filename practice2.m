...
Q=[0	1	2	4	8	16	32	3	5	64	9	6	17	10	18	128	12	33	65	20	256	34	24	36	7	129	66	512	11	40	68	130	19	13	48	14	72	257	21	132	35	258	26	513	80	37	25	22	136	260	264	38	514	96	67	41	144	28	69	42	516	49	74	272	160	520	288	528	192	544	70	44	131	81	50	73	15	320	133	52	23	134	384	76	137	82	56	27	97	39	259	84	138	145	261	29	43	98	515	88	140	30	146	71	262	265	161	576	45	100	640	51	148	46	75	266	273	517	104	162	53	193	152	77	164	768	268	274
...
364	654	659	335	480	315	221	370	613	422	425	451	614	543	235	412	343	372	775	317	222	426	453	237	559	833	804	712	834	661	808	779	617	604	433	720	816	836	347	897	243	662	454	318	675	618	898	781	376	428	665	736	567	840	625	238	359	457	399	787	591	678	434	677	349	245	458	666	620	363	127	191	782	407	436	626	571	465	681	246	707	350	599	668	790	460	249	682	573	411	803	789	709	365	440	628	689	374	423	466	793	250	371	481	574	413	603	366	468	655	900	805	615	684	710	429	794	252	373	605	848	690	713	632	482	806	427	904
...
kernel=[1 0;1 1];
for i=1:n-1
    kernel=kron(kernel,polarkernel);
end
% c=kron(polarkernel,polarkernel);
% polarkernel3=kron(c,polarkernel);
% polarkernel4=kron(polarkernel3,polarkernel);
K=768;%msg=[1 1];
%msg=[1 1 1 1 1 1 1 1];
for j=1:8
    
 
msg = randi([0 1],K,1,'double');
...
dataInMatrix = reshape(enc,length(enc)/k,k);    % Reshape data into binary k-tuples, k = log2(M)
...
%recvmsg=transpose(recvmsg);
[numErrors,ber] = biterr(u,recvmsg);
errors(j)=numErrors;
biterrors(j)=ber;
...
sn=-6:1:1;
figure
plot(sn,biterrors,'LineStyle','--','Marker','.','Color',[0.7,0.5,0.1],'Markersize',20);

%axis([-6 -5 -4 -3 -2 -1 0 1]);
grid on
xlabel('SNR values in dB');
ylabel('Bit Error Rate');
xticklabels({'-60','-50','-40','-30','-20','-10','0','10'})
xtickangle(45)
title('Graphical Represenation of the Bit Error Rate as the SNR changes')

figure
plot(sn,errors,'Marker','*','Color',[0,0.7,0.9],'Markersize',18);
...
xticklabels({'-60','-50','-40','-30','-20','-10','0','10'})
xtickangle(45)
title('Graphical Represenation of the Number of Errors as the SNR changes')

disp('Original message was: ');
disp(u);
disp('Received message was: ');
disp(recvmsg);
